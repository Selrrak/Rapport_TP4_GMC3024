...
\begin{center}
	...
	Couple du moteur & $T_{moteur}$ & N/A [Nm]\\
...
\[ T_c = \frac{m*g*P}{2\pi*\varepsilon} = 9,7 \]
  \item Transmission de torque entre la roue dentée et le pinion
\[ T_f = \frac{1}{2}f*m*g*R_g= \]
\[ T_b = \eta*\frac{Z_b}{Z_a}*T_a = 17,37 \]
...
\[ T_{moteur} = T_{worm} = \frac{T_b*Z_w}{e_w*Z_b} = 3,57 > 2,1 Nm \]
Couple trop grand, il faut diminuer la charge sur le système
\end{enumerate}
	\subsubsection{couple pour soulever une charge optimale (200 lbs)}
...
\subsection{Calcul \#5: rupture du mur de la vis sans fin (pièce \#14)}
\subsection{Calcul \#6: force nécessaire pour maintenir la vis de pressin (pièce \#11)}
\subsection{Calcul \#7: flambage de la vis ACME (pièce \#7)}
